...
export function DoctorLogin() {
  ...
  const handleRequestOTP = async (e) => {
    ...
    try {
      ...
      if (!doctorKey || doctorKey.trim().length < 6) {
        throw new Error('Please enter a valid doctor key')
      }

      console.log('🔐 Starting Doctor Login...')
      const response = await api.loginDoctor(phone, doctorKey)
  ...
  const handleVerify = async (otpValue) => {
    ...
    try {
      ...
      const response = await api.verifyDoctorOTP(userId, otpValue)
        ...
        <div className="max-w-md w-full">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 p-6 sm:p-8">
            ...
            <div className="text-center mb-6">
              ...
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Doctor Login</h1>
              <p className="text-gray-600 dark:text-gray-400">Access your practice and patient records</p>
            </div>
            ...
            {step === 'phone' ? (
              <form onSubmit={handleRequestOTP} className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Doctor Key</label>
                  <input
                    type="password"
                    value={doctorKey}
                    onChange={(e) => setDoctorKey(e.target.value)}
                    className="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 dark:focus:ring-green-400 focus:border-transparent transition-colors"
                    placeholder="Enter your doctor key (password)"
                    disabled={isLoading}
                    required
                  />
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">You received this key after verification approval</p>
                </div>
            ...
            <div className="text-center mt-6">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Not registered yet?</p>
              <button
                type="button"
                onClick={() => route('/doctor/register')}
